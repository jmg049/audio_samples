...
use crate::{
    ...
    AudioTypeConversion, CastFrom, CastInto, ConvertTo, I24, RealFloat,
    ...
    f64: ConvertTo<T>,
{
    ...
    for i in 0..samples_per_channel {
        for ch in channels {
            interleaved.push(ch.data.as_slice().unwrap()[i]);
        }
    }
...
fn interleave_i16_simd_impl<'a>(
    ...
    if num_channels == 2 {
        let left_data = channels[0].data.as_slice().unwrap();
        let right_data = channels[1].data.as_slice().unwrap();
