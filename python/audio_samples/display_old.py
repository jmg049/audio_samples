...
def _font_rc(options: WaveformPlotOptions | SpectrogramPlotOptions | ComparisonPlotOptions) -> dict[str, Any]:
    if options.use_tex:
        return {
            "text.usetex": True,
            "font.family": "serif",
...
def specshow(
    ...
    * `n_fft` - FFT size used in STFT (for frequency axis)
    ...
    * `save_path` - Path to save figure (optional)
    ...
    if data.ndim == 2:
        im = plt.imshow(data, 
                       ...
                       origin='lower',
                       cmap=cmap,
                       extent=[time_coords[0], time_coords[-1], 
                              freq_coords[0], freq_coords[-1]])
    else:
        im = plt.plot(time_coords, data)
        
    # Apply dB range if specified
    if db_range is not None and data.ndim == 2:
        im.set_clim(db_range)
    
    # Set axis properties
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    if yscale == "log" and data.ndim == 2:
        plt.yscale('log')
    
    plt.title(title)
    
    # Add colorbar for 2D data
...
def spectrogram(
    ...
    n_fft = options.window_size or options.n_fft  # Legacy window_size takes precedence if specified
    hop_length = options.hop_size or options.hop_length or n_fft // 4  # Legacy hop_size takes precedence
    ...
    for ch in range(num_channels):
        ...
        if options.freq_scale == "log":
            ax.set_yscale('log')
            ax.set_ylabel(f"Channel {ch + 1} - {options.ylabel} (log)" if num_channels > 1 else f"{options.ylabel} (log)")
        else:
            ax.set_ylabel(f"Channel {ch + 1} - {options.ylabel}" if num_channels > 1 else options.ylabel)
...
def plot_difference(
    ...
    min_length = min(len(audio1.extract_channel(0).numpy()), 
                     len(audio2.extract_channel(0).numpy()))
    ...
    for ch in range(num_channels):
        ...
        ax.plot(time_axis, diff, color=options.wave_color, 
                linewidth=options.wave_linewidth, alpha=options.wave_alpha)
